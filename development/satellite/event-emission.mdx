...
**Purpose:** Update instance status in real-time (per-user)
...
```typescript
{
  ...
  team_id: string;
  user_id: string;  // NEW: Identifies which user's instance
  status: 'awaiting_user_config' | 'provisioning' | 'command_received' | 'connecting'
    | 'discovering_tools' | 'syncing_tools' | 'online' | 'restarting' | 'offline'
    | 'error' | 'requires_reauth' | 'permanently_failed';
...
eventBus.emit('mcp.server.status_changed', {
  ...
  team_id: 'team_xyz',
  user_id: 'user_alice',  // Alice's instance
  status: 'online',
...
```typescript
{
  installation_id: string;
  team_id: string;
  user_id: string;  // Per-user instance logs
  logs: Array<{
...
eventBus.emit('mcp.server.logs', {
  installation_id: 'inst_abc123',
  team_id: 'team_xyz',
  user_id: 'user_alice',  // Alice's instance logs
  logs: [
    {
      level: 'error',
...
**Backend Action:** Inserts requests into `mcpRequestLogs` table, enforces 100-line limit per user instance
...
<Info>
**Per-User Process Context:** The `process_id` field uniquely identifies each user's process instance using the format `{server_slug}-{team_slug}-{user_slug}-{installation_id}`. This ensures process lifecycle events target the correct user's instance.
...
| Grouping Key | `installation_id + team_id + user_id` | Separate batches per user instance |
...
eventBus.emit('mcp.server.status_changed', {
  ...
  team_id: 'team_456',
  user_id: 'user_alice',  // Per-user instance
  status: 'online',
...
- [Status Tracking](/development/satellite/status-tracking) - Per-user instance status values and lifecycle
- [Instance Lifecycle](/development/satellite/instance-lifecycle) - Four lifecycle processes for instance management
- [Log Capture](/development/satellite/log-capture) - Logging system details
- [Process Management](/development/satellite/process-management) - Per-user process lifecycle events
