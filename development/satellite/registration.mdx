...
**Behavior:**
- **If set**: Satellite sends explicit URL to backend during registration and first heartbeat
- **If not set**: Backend auto-detects URL from incoming request headers (X-Forwarded-Proto, X-Forwarded-Host)
...
# DEPLOYSTACK_SATELLITE_URL=    # Leave blank or comment out
```

**URL Update Behavior:**
- Registration: URL sent during initial satellite registration
- Startup: URL updated on first heartbeat after satellite restart
- Runtime: URL remains static until satellite restarts
