...
Each route file should follow this **mandatory pattern**:
...
export default async function yourRoute(server: FastifyInstance) {
  server.post('/your-endpoint', {
    ...
    const { name, email } = RequestSchema.parse(request.body);
...
- ✅ **No generic types**: Avoid `<{ Body: RequestInput }>`
- ✅ **preValidation first**: Authorization before schema
- ✅ **Dual schema**: Both `body` and `requestBody` properties
