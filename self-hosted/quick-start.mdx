  ...
  <Step title="Start Satellite Service">
    <Tabs>
      <Tab title="Standard Deployment">
        If you configured the host system with unprivileged user namespaces (recommended):
      ...
      <Tab title="Alternative (--sysctl flag)">
        If you cannot modify the host system, pass the sysctl setting at runtime:
        ...
        <Warning>
        The `--sysctl` flag requires Docker to be run with additional privileges. For production deployments, configuring the host system is preferred.
        </Warning>
      </Tab>
    </Tabs>
